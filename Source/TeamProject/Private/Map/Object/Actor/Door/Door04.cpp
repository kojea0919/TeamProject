...
void ADoor04::BeginPlay()
{
	...
	if (HasAuthority())
	{
		...
		{
			GameModeRef->OnGameStart.AddUObject(this, &ADoor04::InitializeObject);
			GameModeRef->OnGameEnd.AddUObject(this, &ABaseDoor::ResetObject);
		}
	}
}

void ADoor04::ApplyDoorAnimation(float Alpha)
{
	if (DoorMeshComponents.Num() < 2)
	{
		return;
...
void ADoor04::SetupDoorMeshes()
{
	// 문 메시 에셋 로드
	static ConstructorHelpers::FObjectFinder<UStaticMesh> MeshAsset(
		TEXT("/Game/JapaneseSchool/Meshes/WindowDoor/SM_Corridor_Door_02.SM_Corridor_Door_02")
	);
	...
	if (BoxCollision)
	{
		BoxCollision->SetRelativeLocationAndRotation(FVector(0.0f, -10.0f, 100.0f), FRotator::ZeroRotator);
		BoxCollision->SetRelativeScale3D(FVector(2.87262f, 3.832295f, 2.858312f));
	}
}

void ADoor04::CalculateTargetRotations()
{
...
void ADoor04::InitializeObject(EGameMode GameMode)
{
	if (!HasAuthority())
		return;
